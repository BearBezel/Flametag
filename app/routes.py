...
def claim_lighter(token):
    ...
    lighter.public_message = public_message or "🔥 This lighter is owned. If you're holding it… return it 😅"
    lighter.private_message = private_message or "If found, DM the owner and be a legend."
...
def unlock_private(token):
    ...
    if not pin or not lighter.owner_pin_hash or not check_password_hash(lighter.owner_pin_hash, pin):
        flash("Wrong PIN. Nice try, lighter thief 😭", "err")
        return redirect(url_for("main.lighter_page", token=token))
