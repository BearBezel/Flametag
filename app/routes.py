...
def claim_lighter(token):
    ...
    if len(pin) < 4:
        ...
        flash("Pick a PIN with at least 4 characters.", "err")
...
def unlock_private(token):
    ...
    if not lighter.is_claimed():
        ...
        flash("This lighter has not been claimed yet.", "err")

        return redirect(url_for("main.lighter_page", token=token))

    pin = (request.form.get("pin") or "").strip()

    if not pin or not check_password_hash(lighter.owner_pin_hash, pin):

        flash("Incorrect PIN.", "err")
...
def found_lighter(token):
    ...
    if not note:
        ...
        flash("Please leave a short note.", "err")
    ...
    flash("Thanks! Your message has been sent to the owner.", "ok")
...
def admin_generate():
    ...
    how_many = max(0, min(int(request.form.get("how_many") or 0), 5000))
...
def admin_import():
    ...
    tokens = [t.strip().upper() for t in raw.splitlines() if t.strip()]
